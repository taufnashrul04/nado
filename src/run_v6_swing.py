"""
V6 Daily Swing Backtest — multi-symbol, 5 years history.
"""
import sys, time
...
from strategies.swing_daily import signal_fn

# All KuCoin spot pairs — equivalent to Nado base assets we want to swing
...
def main():
    print("=" * 90)
    print(f"V6 DAILY SWING BACKTEST — 5 years history, KuCoin daily")
    print("=" * 90)
    print(f"\n{'Symbol':<14} {'Trades':>7} {'WR%':>6} {'Ret%':>8} {'MaxDD':>8} {'PF':>5} {'Sharpe':>7} {'Hold':>6}")
    print("-" * 80)
    ...
    for sym in SYMBOLS:
        try:
            ...
            avg_hold = (sum((t.exit_ts - t.entry_ts).days for t in r.trades if t.exit_ts) /
                       max(r.num_trades, 1))
            pf = fmt_pf(r.trades)
            print(f"{sym:<14} {r.num_trades:>7} {r.win_rate:>5.1f}% {fmt_pct(r.total_return_pct):>8} "
                  f"{fmt_pct(r.max_drawdown_pct):>8} {pf:>5.2f} {r.sharpe:>7.2f} {avg_hold:>5.1f}d  ({elapsed:.0f}s)")
    ...
    if all_trades:
        ...
        if long_trades:
            l_wr = sum(1 for t in long_trades if t.pnl_usd > 0) / len(long_trades) * 100
            print(f"\nLong only:  {len(long_trades)} trades, WR={l_wr:.1f}%, PF={fmt_pf(long_trades):.2f}")
        if short_trades:
            s_wr = sum(1 for t in short_trades if t.pnl_usd > 0) / len(short_trades) * 100
            print(f"Short only: {len(short_trades)} trades, WR={s_wr:.1f}%, PF={fmt_pf(short_trades):.2f}")

        # Top 3 by WR with min 5 trades
        winners = sorted([r for r in rows if r["trades"] >= 5], key=lambda x: x["wr"], reverse=True)
        print(f"\nTop 5 by WR (min 5 trades):")
        for r in winners[:5]:
            print(f"  {r['sym']:<12} Trades={r['trades']:>3}  WR={r['wr']:.1f}%  Ret={r['ret']:+.2f}%  PF={r['pf']:.2f}  Sharpe={r['sharpe']:.2f}")
