"""
Live Trader V9.1 — AI Stoch+EMA on Nado Perp
=============================================
Settings (per user spec):
  - Max concurrent: 3 positions
  - Margin per trade: $7.00 (fixed)
  - Leverage: MAX per symbol (BTC/ETH 50x, SOL 40x, BNB 20x, DOGE 10x)
...
Risk math (margin $7, lev 50x = $350 notional):
  - Win @ TP partial = +$5.25 (75% of +$7)
  - Loss @ SL = -$4.90 (-70%)
  - Liq buffer @ -100% = -$7.00 max (1% adverse for lev 50x)
"""
...
CONFIG_PATH = Path("/root/nado-bot/config/v9_live.json")
STATE_PATH = Path("/root/nado-bot/state/v9_state.json")
LOG_PATH = Path("/root/nado-bot/logs/v9_trader.log")
TRADE_LOG = Path("/root/nado-bot/logs/v9_trades.jsonl")
...
def status_summary(state, cfg):
    ...
    return (f"📊 *V9 Status*\n"
            ...
            f"Total: {state['total_trades']} trades, WR {wr:.1f}%, ${state['total_pnl_usd']:+.2f}")
