"""
V8 8H Intraday Swing + Daily Trend Filter Backtest.
Hold: max 48h.
"""
import sys, time
...
from strategies.swing_8h_mtf import make_signal_fn_with_daily
...
def main():
    print("=" * 90)
    print(f"V8 8H + DAILY FILTER BACKTEST — {YEARS}y, hold max 48h")
    print("=" * 90)
    ...
    for sym in SYMBOLS:
        try:
            t0 = time.time()
            df_8h = fetch_kucoin_history(sym, TIMEFRAME_8H, years_back=YEARS)
            df_8h = add_indicators(df_8h)
            df_8h.attrs['symbol'] = sym

            df_d = fetch_kucoin_history(sym, TIMEFRAME_D, years_back=YEARS+1)  # extra for warmup
            df_d = add_indicators(df_d)

            signal_fn = make_signal_fn_with_daily(df_d)

            bt = Backtester(sym, TIMEFRAME_8H, initial_balance=130,
                           size_per_trade_pct=0.05, leverage=3.0, fee_pct=0.0005)
            r = bt.run(df_8h, signal_fn)
